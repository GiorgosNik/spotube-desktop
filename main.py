...
def format_song_data(song):
    ...
    info_dict = {'name': name, 'artist': artist, 'album': album, 'year': year, 'duration': duration, 'url': cover_art}
    return info_dict
...
def download_playlist(playlist_url, directory='./'):
    ...
    for song in songs:
        ...
        try:
            ...
            # Edit the ID3 Tags
            set_tags(info_dict, genius)

            # Move to the designated folder
            shutil.move('./' + info_dict['name'] + '.mp3', directory + '/Songs/' + info_dict['name'] + '.mp3')
